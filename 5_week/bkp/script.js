...
function getClearedCells() {
    ...
    for (let i = 0; i < ROWS_COUNT; i++) {
      for (let j = 0; j < COLS_COUNT; j++) {
        if (cells[i][j].isBomb === false && cells[i][j].isCleared === true) {
          clearedCells++;
        }
      }
      //
...
function discoverCell(row, col) {
  ...
  // return if the cell has already been discovered
  if (!cell.discovered && !cell.hasBeenFlagged) {
    cell.discovered = true;

    if (cell.isBomb) {
      defeat = true;
    } else if (cell.adjacentBombs === 0) {
      // If the cell has no adjacent bombs, discover its neighbors.
...
function flagCell(row, col) {
  var cell = cells[row][col];
  if (!cell.discovered) {
    cell.hasBeenFlagged = !cell.hasBeenFlagged;
  }
  //
...
function countAdjacentBombs(row, col) {
  ...
  function getTotalCellsToClear() {
    var totalCellsToClear = 0;

    for (let i = 0; i < ROWS_COUNT; i++) {
      for (let j = 0; j < COLS_COUNT; j++) {
        if (cells[i][j].isBomb === false && cells[i][j].isCleared === false) {
          totalCellsToClear++;
        }
      }
      //
  ...
  function checkForVictory() {
    var clearedCells = getClearedCells();
    var totalCellsToClear = getTotalCellsToClear();
